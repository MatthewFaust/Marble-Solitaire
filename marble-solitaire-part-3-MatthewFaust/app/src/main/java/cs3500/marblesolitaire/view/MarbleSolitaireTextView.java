...
public class MarbleSolitaireTextView implements MarbleSolitaireView {
   ...
   */
  public MarbleSolitaireTextView(MarbleSolitaireModelState BoardState) {
    if (BoardState == null) {
   ...
   */
  public MarbleSolitaireTextView(MarbleSolitaireModelState BoardState, Appendable ap) {
  ...
  public String toString() {
    ...
    for (int i = 0; i < BoardState.getBoardSize(); i++) {
      for (int j = 0; j < BoardState.getBoardSize(); j++) {
        if (j > 0 && !(j > BoardState.getBoardSize() / 2 &&
                BoardState.getSlotAt(i, j) == MarbleSolitaireModelState.SlotState.Invalid)) {
          board.append(" ");
        }
        if (BoardState.getSlotAt(i, j) == MarbleSolitaireModelState.SlotState.Invalid && j < BoardState.getBoardSize() / 2) {
          board.append(" ");
        } else if (BoardState.getSlotAt(i, j) == MarbleSolitaireModelState.SlotState.Marble) {
          board.append("O");
